  ...
  function Chatbot({
    ...
    const sendMessage = async (
      ...
      const documentQuestion =
        /document|uploaded|file|pdf|txt/i.test(
          backendMessage
        );
      ...
      try {
        ...
        setMessages((previous) => [
          ...previous,
          {
            sender: "bot",
            text:
              "Unable to connect to the MediLink backend."
          }
        ]);
      } finally {
        setLoading(false);
    ...
    const uploadFile =
      async (event) => {
        ...
        try {
          const response =
            await fetch(
              "http://127.0.0.1:8000/chat/upload",
              {
                method: "POST",
          ...
          setMessages((previous) => [
            ...previous,
            {
              ...
              detail:
                `${data.file_type} • ${data.characters_extracted} characters extracted`
            }
          ]);
    ...
    const suggestions =
      role === "doctor"
        ? [
            patientId
              ? `What is the diagnosis of ${patientId}?`
              : "What is the diagnosis of MED00001?",
            patientId
              ? `What allergies does ${patientId} have?`
              : "What medication is MED00001 taking?",
            "Summarize the uploaded document"
          ]
        : [
            patientId
              ? `When is ${patientId}'s next appointment?`
              : "When is MED00001's next appointment?",
            patientId
              ? `What insurance does ${patientId} have?`
        ...
        <div className="chat-scroll">
          ...
          {messages.length === 0 && (
            <div className="chat-empty">
              Start a new MediLink AI conversation.
            </div>
          )}
